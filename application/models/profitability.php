...
class profitability extends Model
{
    ...
    function getReportsMiesieczne()
    {
        ...
        $where = " where a.rowid!=0 and a.activity=1 and a.rozliczenie='miesieczne'";
        ...
        $query = "
            ...
            (agreements a left outer join printers b on a.serial=b.serial)
                ...
                left outer join (SELECT sum(ilosc_km) as 'ilosc_km', sum(czas_pracy) as 'czas_pracy', sum(wartosc_materialow) as 'wartosc_materialow',nn.serial as 'serial'
                                 FROM `notifications` as nn                                 
                                 WHERE DATE(date_zakonczenia) >= '{$this->dataod}'
                                 and DATE(date_zakonczenia) <= '{$this->datado}'
                                 group by serial) n on a.serial = n.serial 
                and c.activity=1
