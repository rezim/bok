...
class devicecountersController extends Controller
{
    ...
    function showdaneklient()
    {
        ...
        foreach ($dataReportsMiesieczne as $key => $item) {
            ...
            $isPrinter = $item['typ_umowy'] === 'wynajem drukarki';
            $isScanner = $item['typ_umowy'] === 'wynajem skanera';

            if (!$isPrinter && !$isScanner) {
                continue;
            }

            $blackDate = explode(" ", $item['data_wiadomosci_black_koniec'])[0];
            $colorDate = explode(" ", $item['data_wiadomosci_kolor_koniec'])[0];
            $scansDate = explode(" ", $item['data_wiadomosci_scans_koniec'])[0];

            $blackEndDate = $item['data_wiadomosci_black_koniec'];
            $colorEndDate = $item['data_wiadomosci_kolor_koniec'];
            ...
            if (
                ...
                $dataReports[$item['rowidclient']]['umowy'][$item['rowidumowa']]['skany_start'] = $item['skany_start'];
                $dataReports[$item['rowidclient']]['umowy'][$item['rowidumowa']]['data_wiadomosci_scans_start'] = $item['data_wiadomosci_scans_start'];
                $dataReports[$item['rowidclient']]['umowy'][$item['rowidumowa']]['skany_koniec'] = $item['skany_koniec'];
                $dataReports[$item['rowidclient']]['umowy'][$item['rowidumowa']]['data_wiadomosci_scans_koniec'] = $item['data_wiadomosci_scans_koniec'];

                $dataReports[$item['rowidclient']]['umowy'][$item['rowidumowa']]['type_color'] = $item['type_color'];
                // device localization
